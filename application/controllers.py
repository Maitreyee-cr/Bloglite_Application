...
def login():
    ...
    if form.validate_on_submit():
        ...
        ur = User.query.filter(User.Username == username).first()
        if ur:
            if bcrypt.check_password_hash(ur.Password,password):
                login_user(ur)
...
def register():
    ...
    if form.validate_on_submit():
        print("registering..")
        h_pword = bcrypt.generate_password_hash(form.password.data)
        new_user = User(Username = form.username.data, Password = h_pword,email = form.email.data,About = "")
...
@login_required
def profile():
    b_list = current_user.Blogs
    b_list.sort(reverse = True, key = time)
    return render_template("profile.html",b_list = b_list)
...
def home():
    ...
    for ur in current_user.Following:
        following.append(ur.id)
...
def search():
    form = SearchForm()
    s_query = request.args.get("search")
    if not s_query:
        return redirect("/home")
    users = User.query.filter(User.Username.like('%'+s_query+'%')).all()
    return render_template('search_results.html',users=users,form = form,s_query=s_query)
    ...
    if(user):
        ...
        if current_user.is_authenticated:
            ...
            if user in current_user.Following:  
                following = True
        return render_template("userpage.html",user=user,following = following,b_list=b_list)
    return redirect('/home')
...
def edit_blog(blog_id):
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if(os.path.exists(image_path)):
                os.remove(image_path)
                print("Old image has been deleted")
            else:
